...
async def _handle_stat(bot, event: MessageEvent):
    ...
    for src in source_names:
        if src == "todayhit":
            cnt = Article.select().where(
                (Article.source == src) | Article.source.is_null()
            ).count()
        else:
            cnt = Article.select().where(Article.source == src).count()
        if cnt > 0:
            lines.append(f"  {_source_label(src)}: {cnt} 条")
    ...
    lines.extend(["", f"有部门信息: {with_dept}", f"有时间信息: {with_time}", sep])
    await bot.send(event, "\n".join(lines))
